...
import { WHATSAPP_PHONE } from '@/lib/env';
...
export default async function ProductPage({ params }: Props) {
    ...
    try {
        ...
        // Cargar en paralelo galería, variantes y config
        const [gallery, variants, config] = await Promise.all([
        ...
        // JSON-LD (estático al min price del producto)
        const jsonLd = {
        ...
        return (
            <>
                <Breadcrumbs items={[{ label: 'Inicio', href: '/' }, { label: 'Catálogo', href: '/' }, { label: p.name, href: `/producto/${p.slug}` }]} />
                    ...
                    <div className="space-y-4">
                        ...
                        {p.min_price_visible != null ? (
                            <p className="text-xl font-medium">
                                {/* precio “desde” */}
                                Desde {new Intl.NumberFormat('es-UY', {
                                    style: 'currency',
                                    currency: (config.currency_code || 'UYU') as any,
